...
const InventoryModal: React.FC<InventoryModalProps> = ({
  ...
  const relevantBlocks = Array.isArray(blocks)
    ? blocks.filter((block) => {
        // Apply blockName filter
        const matchesBlockName = block.blockType.blockName === blockName;
        // Apply processed filter
        const isNotProcessed = block.processed === false;
        // Apply supplierId filter only if supplierId is provided
        const matchesSupplierId =
          supplier === "" || block.suppliers.supplierName === supplier;

        return matchesBlockName && isNotProcessed && matchesSupplierId;
      })
    : [];
